...
import { Club, UserClub } from "@/types";
...
const ClubsPage = () => {
  ...
  const fetchClubs = async () => {
    setClubs([]);
    try {
      const data = await getJSON<{ clubs: Club[] }>("/api/clubs", { search: searchTerm });
      setClubs(data.clubs ?? []);
    } catch {
  ...
  async function fetchUserClub() {
    ...
    setUserClub(null);
    try {
      const data = await getJSON<UserClub>("/api/clubs/user", { uid: user.uid });
      setUserClub(data);
    } catch {
  ...
  const handleJoinClub = async (clubName: string) => {
    ...
    setError(null);
    try {
      await postJSON(`/api/clubs/${clubName}/members`, { uid: user.uid });
  ...
  const handleLeaveClub = async (clubName: string) => {
    ...
    setError(null);
    try {
      await deleteJSON(`/api/clubs/${clubName}/members`, { uid: user.uid });
      await Promise.all([fetchUserClub(), fetchClubs()]);
    } catch {
